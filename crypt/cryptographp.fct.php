...
// -----------------------------------------------
 ...
 // CMB: changed to return the (x)html
function dsp_crypt($cfg = 0, $reload = 1) {
    // Affiche le cryptogramme
    $htm = '<table><tr><td><img id="cryptogram" src="'.$_SESSION['cryptdir'].'/cryptographp.php?cfg='
            .$cfg.'&'.SID.'"></td>';
    if ($reload) {
        $htm .= '<td><a title="'.($reload == 1 ? '' : $reload).'" style="cursor:pointer"'
                .' onclick="javascript:document.images.cryptogram.src=\''.$_SESSION['cryptdir'].'/cryptographp.php'
                .'?cfg='.$cfg.'&'.SID.'&\'+Math.round(Math.random(0)*1000)+1">'
                .'<img src="'.$_SESSION['cryptdir'].'/images/reload.gif"></a></td>';
    }
    $htm .= '</tr></table>';
    return $htm;
}
